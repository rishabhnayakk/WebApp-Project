...
export const isAdminIdentifier = (identifier) => {
  ...
  return (
    (cfg.adminId && q === cfg.adminId.toLowerCase()) ||
...
const buildAdminUser = (cfg, inputId) => ({
  id: cfg.adminId,
...
const buildAdminResponse = (cfg, inputId) => ({
  ...buildAdminUser(cfg, inputId),
...
const buildUserPublicShape = (user) => ({
  id: user.id,
  name: user.name,
  email: user.email,
...
export const AUTH_COOKIE_OPTIONS = {
  ...
  sameSite: 'lax',
  maxAge: 100 * 365 * 24 * 60 * 60 * 1000, // 100 years — never expires automatically
};
...
/**
 * Finds an existing user by googleId/email, or registers a new user automatically.
 * Used by both Passport.js Google OAuth 2.0 and the GIS token verification route.
 */
...
router.get('/config', (req, res) => {
  ...
  const isConfigured = Boolean(
    googleClientId && googleClientSecret && !googleClientId.includes('YOUR_GOOGLE_CLIENT_ID')
  );
...
// Passport.js Google OAuth 2.0 — Initiate Sign-In
...
// Passport.js Google OAuth 2.0 — Callback
    ...
    try {
      ...
      if (req.query.state) {
        try {
          ...
          if (parsed?.returnTo) returnTo = sanitizeRedirectUrl(parsed.returnTo, '/account.html');
        } catch (_) {
      ...
      const destination = isAdmin ? '/admin.html' : returnTo;
      const sep = destination.includes('?') ? '&' : '?';

      return res.redirect(
        `${destination}${sep}token=${encodeURIComponent(token)}&google_auth=success&name=${encodeURIComponent(user.name)}`
      );
    } catch (tokenErr) {
...
router.post('/admin-login', rateLimiter(30, 60000), (req, res) => {
  ...
  if (!isMatch) {
    logAudit('ADMIN_DIRECT_LOGIN_FAILED', { adminId: inputId, ip: getIp(req) });
    return res.status(401).json({ success: false, message: 'Invalid admin credentials.' });
  }

  const adminUser = buildAdminUser(cfg, inputId);
...
// Email lookup (check if account exists)
router.post('/email-lookup', rateLimiter(30, 60000), (req, res) => {
  ...
  return res.json({ success: true, exists: false, email: cleanInput, name: null, role: 'CUSTOMER' });
});
...
router.post('/login', rateLimiter(30, 60000), (req, res) => {
  ...
  if (isAdminIdentifier(q) && password.trim() === cfg.adminPassword) {
    const adminUser = buildAdminUser(cfg, q);
    const token = generateToken(adminUser);
    res.cookie('aerosol_token', token, AUTH_COOKIE_OPTIONS);
  ...
  if (!user || user.password !== password) {
    logAudit('LOGIN_FAILED', { email: q, ip: getIp(req) });
  ...
  res.json({
    ...
    user: buildUserPublicShape(user),
  });
});
...
router.post('/register', rateLimiter(10, 60000), (req, res) => {
  ...
  if (!termsAccepted)
    return res.status(400).json({ success: false, message: 'Please agree to the Terms & Conditions.' });

  if (!privacyAccepted)
    return res.status(400).json({ success: false, message: 'Please accept the Privacy Policy.' });
...
// Google Sign-In (GIS token verification)
router.post('/google', rateLimiter(20, 60000), async (req, res) => {
  ...
  if (credential) {
    const clientId = (process.env.GOOGLE_CLIENT_ID || '').trim();
    if (!clientId) {
      logAudit('GOOGLE_LOGIN_ERROR_NOCLIENTID', { ip: getIp(req) });
    ...
    try {
      ...
      if (!payload || !payload.sub || !payload.email) {
        logAudit('GOOGLE_TOKEN_INVALID_PAYLOAD', { ip: getIp(req) });
        return res.status(401).json({ success: false, message: 'Invalid Google credential token payload.' });
      }
      ...
      logAudit('GOOGLE_TOKEN_VERIFY_FAILED', { error: verifyError.message, ip: getIp(req) });
    ...
    logAudit('GOOGLE_DEV_MOCK_LOGIN', { email: verifiedGoogleData.email, ip: getIp(req) });
  } else {
    return res.status(400).json({ success: false, message: 'Google credential token is required for authentication.' });
  }
  ...
  return res.json({
    ...
    token,
    redirectUrl: isAdmin ? '/admin.html' : safeRedirect,
    user: {
      id: user.id,
      name: user.name,
      email: user.email,
...
router.delete('/account', requireAuth, (req, res) => {
  ...
  logAudit('ACCOUNT_DEACTIVATED', { userId: req.user.id, email: user.email });
...
router.put('/addresses', (req, res) => {
  ...
  const user = usersDB.find((u) => u.email.toLowerCase() === email.trim().toLowerCase());
  if (user) user.addresses = Array.isArray(addresses) ? addresses : [];
  ...
  try {
    ...
    const logins = logs
      ...
      .map((l) => {
        ...
        const isAdminEvent = l.eventType.includes('ADMIN');
        const isGoogleEvent = l.eventType.includes('GOOGLE');

        return {
          id: l.id,
          name: d.name || (matchedUser ? matchedUser.name : isAdminEvent ? 'Operations Administrator' : 'Customer User'),
          phone: d.phone || (matchedUser?.phone ? matchedUser.phone : 'Not provided'),
          email: d.email || (matchedUser ? matchedUser.email : d.adminId || 'N/A'),
          role: d.role || (matchedUser ? matchedUser.role : isAdminEvent ? 'ADMIN' : 'CUSTOMER'),
          authMethod: d.authMethod || (isAdminEvent ? 'Admin Login' : isGoogleEvent ? 'Google OAuth' : 'Email & Password'),
          timestamp: d.loginTime || l.timestamp,
          ip: d.ip || '127.0.0.1',
        };
      });

    res.json({ success: true, total: logins.length, data: logins });
  } catch (err) {
